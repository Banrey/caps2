...
if (@$_GET["action"] == "delete"){
	$sql_delete = "DELETE FROM comments WHERE commentID = ?";
	...
	if (empty($_POST["commentID"])){
		$sql_check = "INSERT INTO comments (postID, accID, message) VALUES (?, ?, ?)";
		...
		$sql_check = "UPDATE comments SET postID = ?, accID = ?, message = ? WHERE commentID = ?  ";
		if ($statement_check = mysqli_prepare($conn, $sql_check)){
